...
public class OrdineAcqTuttiBulk extends OrdineAcqBulk{
    @Override
    public OggettoBulk initializeForSearch(CRUDBP bp, ActionContext context) {
        ...
        ordineAcqBulk.getNumerazioneOrd().setEsercizio(it.cnr.contab.utenze00.bulk.CNRUserInfo.getEsercizio(context));
        Unita_organizzativaBulk uo = it.cnr.contab.utenze00.bulk.CNRUserInfo.getUnita_organizzativa(context);
        if (uo.isUoEnte())
            ordineAcqBulk.setCdCds(null);
