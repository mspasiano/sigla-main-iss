...
(P_ESERCIZIO IN ESERCIZIO.ESERCIZIO%TYPE,
	P_TIPO IN VARCHAR2,
	P_NUM_LIV IN NUMBER,
	P_TIPO_AGGREGAZIONE IN VARCHAR2,
	P_TIPO_RENDICONTO IN VARCHAR2,
	P_ORIGINE IN VARCHAR2 DEFAULT 'REA',
	...
	P_VOCE_DA IN VARCHAR2 DEFAULT NULL,
...
BEGIN
	...
	THEN
		...
		-- rec rappresenta una tupla della vista `v_stampa_bilancio_rendic_x_cdr`
		Update PDG_DATI_STAMPA_RENDIC_TEMP
		...
		-- se non ci sono delle righe nella vista allora imposto tutto a 0
	...
	Else
		...
		FOR PREV_COMP IN (SELECT esercizio, tipo,
			...
			AND tipo = P_TIPO
			AND ((P_VOCE_DA IS NULL) OR (TO_NUMBER(CD_LIVELLO6) >= DECODE(P_VOCE_DA, '*', 1, TO_NUMBER(P_VOCE_DA))))
            AND ((P_VOCE_A IS NULL) OR (TO_NUMBER(CD_LIVELLO6) <= DECODE(P_VOCE_A, '*', 9999, TO_NUMBER(P_VOCE_A))))
			GROUP BY
			esercizio,
			tipo,
			cd_livello1,
		...
		If (P_COMPLETA = 'Y') Then
			If (P_TIPO = 'S') Then
				PRC_LOAD_TABLE_STAMPA_RENDIC(P_ESERCIZIO, 'E', P_NUM_LIV, P_TIPO_AGGREGAZIONE, P_TIPO_RENDICONTO, P_ORIGINE, 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'SYSTEM', P_VOCE_DA, P_VOCE_A);
			Else
				PRC_LOAD_TABLE_STAMPA_RENDIC(P_ESERCIZIO, 'S', P_NUM_LIV, P_TIPO_AGGREGAZIONE, P_TIPO_RENDICONTO, P_ORIGINE, 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'SYSTEM', P_VOCE_DA, P_VOCE_A);
			End If;
		End If;
	End If;
End;
