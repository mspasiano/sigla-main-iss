...
BEGIN
	...
	BEGIN
		SIGLASVILUPPO.INSERISCI_LOG('Valore P_VOCE_DA: ' || P_VOCE_DA || ', Valore P_VOCE_A: ' || P_VOCE_A);
	END;
	...
	Else
		...
		FOR PREV_COMP IN (SELECT esercizio, tipo,
			...
			AND tipo = P_TIPO
			AND ((P_VOCE_DA IS NULL) OR (P_VOCE_DA <= CD_LIVELLO6))
			AND ((P_VOCE_A IS NULL) OR (P_VOCE_A >= CD_LIVELLO6))
			GROUP BY
			esercizio,
			tipo,
			cd_livello1,
