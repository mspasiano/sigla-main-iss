...
--------------------------------------------------------
  ...
  Select reversale.cd_cds, reversale.esercizio, reversale.pg_reversale,
            ...
            NULL dt_invio_dis,(select decode(max(cd_modalita_pag),min(cd_modalita_pag),max(cd_modalita_pag),null) from reversale_riga
            where   reversale_riga.cd_cds   = reversale.cd_cds And
                               reversale_riga.esercizio  = reversale.esercizio And
                               reversale_riga.pg_reversale = reversale.pg_reversale) cd_modalita_pag,esito_operazione,dt_ora_esito_operazione,errore_siope_plus
      From  reversale
  ...
  -- MANDATI IN DISTINTA
     Select reversale.cd_cds, reversale.esercizio, reversale.pg_reversale,
            ...
            (select decode(max(cd_modalita_pag),min(cd_modalita_pag),max(cd_modalita_pag),null) from reversale_riga
            where   reversale_riga.cd_cds   = reversale.cd_cds And
                               reversale_riga.esercizio  = reversale.esercizio And
                               reversale_riga.pg_reversale = reversale.pg_reversale) cd_modalita_pag,
            esito_operazione,
            dt_ora_esito_operazione,
            errore_siope_plus
