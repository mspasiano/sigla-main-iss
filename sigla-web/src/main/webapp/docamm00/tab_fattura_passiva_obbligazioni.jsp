...
<%	CRUDFatturaPassivaBP bp = (CRUDFatturaPassivaBP)BusinessProcess.getBusinessProcess(request);
	...
	<div class="Group card" style="width:100%">
		<table width="100%">
			<tr>
			  	<td colspan="3">
				  	<span class="FormLabel text-info h3" style="color:black">Impegni</span>
			  	</td>
			</tr>
			<tr>
			  	<td colspan="3">
					<%	boolean canAdd = true;
						...
						if (fatturaPassiva instanceof Nota_di_creditoBulk || fatturaPassiva instanceof Nota_di_debitoBulk)
							canAdd = false;
						bp.getObbligazioniController().writeHTMLTable(pageContext,"default",canAdd,false,!fatturaPassiva.isDaOrdini(),"100%","auto;max-height: 40vh;"); %>
			  	</td>
			</tr>
			<tr>
			  	<td>
		   			<% bp.writeFormLabel(out,"importoTotalePerObbligazione"); %>
				  	<% bp.writeFormInput(out,"importoTotalePerObbligazione");%>
			  	</td>
			  	<td>
			...
			<tr>
			  	<td>
		   			<%	Obbligazione_scadenzarioBulk obbl = (Obbligazione_scadenzarioBulk)bp.getObbligazioniController().getModel();
		   				if (obbl == null) { %>
						  	<span class="FormLabel text-info" style="color:black">Dettagli <%=(fatturaPassiva instanceof Nota_di_creditoBulk)?"nota di credito stornati":(fatturaPassiva instanceof Nota_di_debitoBulk)?"nota di debito addebitati":"fattura contabilizzati"%></span>
		   			<%	} else { %>
						  	<span class="FormLabel text-info" style="color:black">Dettagli <%=(fatturaPassiva instanceof Nota_di_creditoBulk)?"nota di credito stornati":(fatturaPassiva instanceof Nota_di_debitoBulk)?"nota di debito addebitati":"fattura contabilizzati"%> su "<%=obbl.getDs_scadenza()%>"</span>
		   			<%	} %>
			  	</td>
			</tr>
			<tr>
			  	<td>
					<% bp.getDettaglioObbligazioneController().writeHTMLTable(pageContext,"righiSet",!fatturaPassiva.isDaOrdini(),false,true,"100%","auto"); %>
			  	</td>
			</tr>
   		</table>
	</div>